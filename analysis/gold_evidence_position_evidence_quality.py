...
def main():
    ...
    pivot_table.to_csv(base_data_path / 'tables' / 'gold_evidence_position_evidence_quality.csv')

    latex_table = format_results_to_latex(pivot_table)
    with open(base_data_path / 'tables' / 'gold_evidence_position_evidence_quality.tex', 'w') as f:
        f.write(latex_table)
    ...
    correlation_table.to_latex(
        base_data_path / 'tables' / 'gold_evidence_position_evidence_quality.tex',
        escape=False,
...
def make_summary_table_and_plot(
    ...
    # Save plot
    plt.tight_layout()
    plt.savefig(infer_base_data_path() / 'plots' / 'gold_evidence_position_evidence_quality_summary.pdf')

    # Aggregate difference_to_gold by averaging
    aggregated_table_mean = position_performance_results_table.groupby(['model', 'description', 'bin_idx'])[
        'diff_to_mean'].mean()
    aggregated_table_std = position_performance_results_table.groupby(['model', 'description', 'bin_idx'])[
        'diff_to_mean'].std()
    ...
    aggregated_table.to_latex(
        infer_base_data_path() / 'tables' / 'gold_evidence_position_evidence_quality_summary_table',
        escape=False,
        float_format='%.2f'
    )
    # Output summary table as csv
    aggregated_table.to_csv(
        infer_base_data_path() / 'tables' / 'gold_evidence_position_evidence_quality_summary_table.csv',
        index=True
    )
...
def make_lineplots(position_performance_results_table):
    ...
    position_performance_results_table['model_description'] = position_performance_results_table['model'] + ' ' + \
                                                              position_performance_results_table['description']
    ...
    fig.canvas.draw()
    plt.tight_layout()
    plt.savefig(infer_base_data_path() / 'plots' / 'gold_evidence_position_evidence_quality_lineplots.pdf')
